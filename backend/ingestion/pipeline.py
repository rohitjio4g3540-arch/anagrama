...
def ingest(title: str, content: str, kind: str = "document", metadata: dict | None = None, user_id: str | None = None) -> dict:
...
def ingest_file(path: Path, filename: str, user_id: str | None = None) -> dict:
    content = path.read_text(encoding="utf-8", errors="replace")
    return ingest(filename, content, kind="file", metadata={"path": str(path)}, user_id=user_id)
