...
async def build_context(message: str, project_id: str | None = None, user_id: str | None = None) -> AssembledContext:
    nodes = await graph_context(message, user_id)
    sources = await search(message, user_id)
